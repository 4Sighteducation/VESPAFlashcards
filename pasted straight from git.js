   ...
   function extractValidRecordId(value) {
    ...
    window.initializeFlashcardApp = function() {
      ...
      if (!window.VESPA_CONFIG) { 
        console.error("Flashcard app: Missing required VESPA_CONFIG.");
        return; // Exit if the main config object isn't even set
      }
        const config = window.VESPA_CONFIG; // Assign for easier access
        
      if (!config.appUrl || !config.elementSelector) { // Add checks for necessary config properties
        console.error("Flashcard app: VESPA_CONFIG is missing required appUrl or elementSelector.", config);
        return;
      }
      const FLASHCARD_APP_URL = config.appUrl; // Define URL from config
      const APP_CONTAINER_SELECTOR = config.elementSelector; // Define selector from config
