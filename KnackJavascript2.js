...
function verifyDataSave(recordId) {
          ...
          try {
            ...
            if (Array.isArray(topicLists) && topicLists.length > 0) {
              ...
              topicLists.forEach((list, index) => {
                ...
                // Check if topics array has valid structure
                if (Array.isArray(list.topics)) {
                  list.topics.forEach((topic, topicIndex) => {
                    if (!topic.id) {
                      isValidStructure = false;
                      validationErrors.push(`Topic ${topicIndex} in list ${index} missing id`);
                    }
                    if (!topic.name) {
                      isValidStructure = false;
                      validationErrors.push(`Topic ${topicIndex} in list ${index} missing name`);
                    }
                  });
                }
              });
              ...
              if (!isValidStructure) {
                console.error(`Flashcard app [${new Date().toISOString()}]: Topic lists structure validation failed:`, validationErrors);
                // Try to fix the structure if possible
                const fixedTopicLists = topicLists.map(list => ({
                  id: list.id || generateId('list'),
                  subject: list.subject || 'Unknown',
                  examBoard: list.examBoard || 'Unknown',
                  examType: list.examType || 'Unknown',
                  topics: Array.isArray(list.topics) ? list.topics.map(topic => ({
                    id: topic.id || generateId('topic'),
                    name: topic.name || 'Unknown Topic'
                  })) : []
                }));
                
                // Save the fixed structure back to Knack
                $.ajax({
                  url: KNACK_API_URL + '/objects/' + FLASHCARD_OBJECT + '/records/' + recordId,
                  type: 'PUT',
                  headers: {
                  ...
                  data: JSON.stringify({
                    [FIELD_MAPPING.topicLists]: JSON.stringify(fixedTopicLists)
                  }),
                  success: function() {
                    console.log(`Flashcard app [${new Date().toISOString()}]: Successfully fixed and saved topic lists structure`);
                  },
                  error: function(error) {
                    console.error(`Flashcard app [${new Date().toISOString()}]: Failed to fix topic lists structure:`, error);
                  }
                });
              } else {
                console.log(`Flashcard app [${new Date().toISOString()}]: Topic lists structure validation passed`);
              }
            } else {
  ...
  }, 5000); // Increased to 5 seconds to ensure data is committed
}
