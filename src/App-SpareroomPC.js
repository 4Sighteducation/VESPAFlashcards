...
function App() {
  ...
  const handleGenerateCardsFromTopic = useCallback((topicData) => {
    // Check if we have a full metadata object or just a topic string
    if (typeof topicData === 'object') {
      // We have metadata - store these values for the AI generator
      setSelectedTopic(topicData.topic);
      setSelectedSubject(topicData.subject);
      setTopicListExamBoard(topicData.examBoard);
      setTopicListExamType(topicData.examType);
    } else {
      // Legacy format - just a topic string
